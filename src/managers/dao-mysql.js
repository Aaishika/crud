...
DaoMysql.prototype.list = function(itemClass, propNames, callback) {
    var that = this;
    if (this.cache) {
        this.cache.getItems(itemClass, function(cachedErr, cachedResult) {  
            if (cachedErr || !cachedResult) {
                ...
                that.connection.query(queryStr, function(err, results, fields) {
                    if (err) {
                        ...
                        that.cache.putItems(itemClass, results);          
                    }
                    if (callback) {
...
DaoMysql.prototype.get = function(itemClass, itemId, callback) {
    var that = this;
    if (this.cache) {
        this.cache.getItem(itemClass, itemId, function(cachedErr, cachedResult) {  
            if (cachedErr || !cachedResult) {
                ...
                that.connection.query(queryStr, [ itemId ], function(err, result, fields) {
                    ...
                    if (err) {
                        ...
                        that.cache.putItemByClass(itemClass, itemResult);    
                    }
                    if (callback) {
